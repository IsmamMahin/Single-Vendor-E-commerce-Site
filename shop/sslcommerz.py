...
def generate_sslcommerz_payment(order, request):
    ...
    post_body['success_url'] = request.build_absolute_url(f'/payment/success/{order.id}')
    post_body['fail_url'] = request.build_absolute_url(f'/payment/fail/{order.id}')
    post_body['cancel_url'] = request.build_absolute_url(f'/payment/cancel/{order.id}')
    # post_body['status'] = "Done"
...
def send_order_confirmation_email(order):
    ...
    message = render_to_string('') # html code -> string
    to = order.email
